def partition(a, lo, hi):
    ...
    lt = lo  # We initiate lt to be the part that is less than the pivot
    i = lo  # We scan the array from left to right
    gt = hi  # The part that is greater than the pivot
    pivot = a[lo]  # The pivot, chosen to be the first element of the array, that why we'll randomize the first elements position
    ...
    if a[lt] == a[gt]:
        print("pivot:", pivot, "| lower:", a[lo:lt], "| equal",a[lt:gt+1], "| higher:",a[gt+1:hi+1])
    else :print("lower:", a[lo:lt], "| equal:",a[lt], "| higher:",a[gt+1:hi+1])
